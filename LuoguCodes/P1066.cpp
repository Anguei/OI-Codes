...
math::Unsigned c[505][505], ans = 0;
// int c[505][505], ans = 0;

void solution() {
    int k, w; std::cin >> k >> w;
    rep(i, 0, 504) c[i][0] = c[i][i] = 1;
    rep(i, 2, 504) rep(j, 1, i) c[i][j] = c[i - 1][j - 1] + c[i - 1][j];//, std::clog << "i, j = " << i << " " << j << std::endl;
    rep(i, 2, std::min(w / k, (1 << k) - 1)) ans += c[(1 << k) - 1][i];
    rep(i, 1, std::min((1 << (w % k)), (1 << k) - w / k) - 1) ans += c[(1 << k) - i -1][w / k];
