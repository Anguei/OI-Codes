...
int a[10][10], c[10], f[60];

void solution() {
    rep(i, 0, 9) a[i][i] = 1;
    math::Unsigned ans = 1;
    std::string n; std::cin >> n;
    int k; std::cin >> k; while (k--) { int x, y; std::cin >> x >> y; a[x][y] = 1; }
    rep(k, 0, 9) rep(i, 0, 9) rep(j, 0, 9) a[i][j] |= (a[i][k] & a[k][j]);
    rep(i, 0, 9) rep(j, 0, 9) c[i] += a[i][j];
    rep(i, 0, n.size() - 1) ans = ans * c[n[i] - 48];
