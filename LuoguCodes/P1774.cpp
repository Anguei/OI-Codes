...
namespace BitP1908
{
    ...
    int main()
    {
        ...
        std::sort(nums + 1, nums + n + 1, [](const Num &n1, const Num &n2)
        {
        ...
        for (int i = 1; i <= n; ++i)
        {
            bitP1908.update(nums[i].id, 1, n);
            ans += bitP1908.query(nums[i].id - 1, n);
        }
        std::cout << ans << std::endl;
